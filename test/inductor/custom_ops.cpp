...
Tensor bar_impl(const Tensor& tensor, const int64_t i64) {
  return tensor + i64;
}
...
Tensor bar_meta(const Tensor& tensor, const int64_t i64) {
  return tensor.clone();
}
...
// This custom op has every single support argument type in pytorch.
// This is for testing purposes only.
TORCH_LIBRARY_FRAGMENT(aoti_custom_ops, m) {
  m.def(
  ...
  m.def("bar(Tensor t, int i=3) -> Tensor");
...
TORCH_LIBRARY_IMPL(aoti_custom_ops, CPU, m) {
  m.impl("fn_with_all_inputs", fn_with_all_inputs_impl);
}

TORCH_LIBRARY_IMPL(aoti_custom_ops, CUDA, m) {
  m.impl("fn_with_all_inputs", fn_with_all_inputs_impl);
}

TORCH_LIBRARY_IMPL(aoti_custom_ops, Meta, m) {
  m.impl("fn_with_all_inputs", fn_with_all_inputs_meta);
}
...
TORCH_LIBRARY_IMPL(aoti_custom_ops, CPU, m) {
  m.impl("fn_with_tuple_output", fn_with_tuple_output_impl);
}
...
TORCH_LIBRARY_IMPL(aoti_custom_ops, Meta, m) {
  m.impl("fn_with_tuple_output", fn_with_tuple_output_meta);
}

TORCH_LIBRARY_IMPL(aoti_custom_ops, CPU, m) {
  m.impl("fn_with_list_output", fn_with_list_output_impl);
}
...
TORCH_LIBRARY_IMPL(aoti_custom_ops, Meta, m) {
  m.impl("fn_with_list_output", fn_with_list_output_meta);
}

TORCH_LIBRARY_IMPL(aoti_custom_ops, CPU, m) {
  m.impl("fn_with_mix_outputs", fn_with_mix_outputs_impl);
}
...
TORCH_LIBRARY_IMPL(aoti_custom_ops, Meta, m) {
  m.impl("fn_with_mix_outputs", fn_with_mix_outputs_meta);
}

TORCH_LIBRARY_IMPL(aoti_custom_ops, CPU, m) {
  m.impl("fn_with_input_mutation", fn_with_input_mutation_impl);
}
...
TORCH_LIBRARY_IMPL(aoti_custom_ops, Meta, m) {
  m.impl("fn_with_input_mutation", fn_with_input_mutation_meta);
}
